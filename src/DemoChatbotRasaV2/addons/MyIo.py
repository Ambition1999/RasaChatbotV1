...
class RestInput(InputChannel):
    ...
    # Lấy sender (conversation_id) từ post request
    async def _extract_sender(self, req) -> Optional[Text]:
        return req.json.get("sender", None)

    # noinspection PyMethodMayBeStatic # Lấy message của user từ post request
    ...
    def blueprint(self, on_new_message: Callable[[UserMessage], Awaitable[None]]):
        ...
        # check status của channel ex: http://localhost:5005/webhooks/myio/
        ...
        async def receive(request: Request):
            ...
            print("Should use stream value is: ",should_use_stream)
